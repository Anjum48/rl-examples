...
do
    ...
    CUDA_VISIBLE_DEVICES='' python3 -m dppo.dppo_lstm --timestamp=${DATETIME} --job_name="ps" --task_index=${p} \
...
do
    ...
    CUDA_VISIBLE_DEVICES='' python3 -m dppo.dppo_lstm --timestamp=${DATETIME} --job_name="worker" --task_index=${w} \
    --workers=${N_WORKER}  --agg=${N_AGG} --ps=${PS} &>/dev/null &
    w=$(( $w + 1 ))
done